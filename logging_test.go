...
func Test_requestLogger(t *testing.T) {
	...
	const expectedLogString = `time=2023-10-01T12:34:57.000Z level=INFO msg="Served request" method=GET path=/stats client_ip=192.0.2.1:1234` + "\n"
	const expectedStatusCode = http.StatusOK

	if logBuffer.String() != expectedLogString {
		t.Errorf("got: %s, expected: %s", logBuffer.String(), expectedLogString)
	}
	if rr.Code != expectedStatusCode {
		t.Errorf("got: %d, expected: %d", rr.Code, expectedStatusCode)
	}
